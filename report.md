...
## 境界ポリゴン（SIMA）vs 立面図・辺ごと（SIMA）のLAZ処理比較

| 項目 | 境界ポリゴン（SIMA） | 立面図・辺ごと（SIMA） |
|------|----------------------|-------------------------|
| filterPoint | なし（全点書き込み） | あり（範囲クロップポリゴン内のみ） |
| onPoint の役割 | 3領域分類＋Classification/色設定 | 座標変換（transformPointBoundary） |
| streamLAZToLASBlob 呼び出し | 1回 | 辺数 N 回（N 回解凍） |
| 座標系 | オリジナル（数学座標） | 各辺の境界座標系に変換 |

**重要**: `filterPoint` を使うときは、**フィルタ判定を `onPoint`（座標変換）の前**に行う必要がある。範囲クロップポリゴンはオリジナル座標系で定義されているため、変換後の座標で判定すると全点が範囲外になり 0 点になる。

### 座標系の統一

- **点群（LAS/LAZ）**: 入力・出力とも数学座標系
- **SIMA・手入力**: 測量座標系で入力 → `simaToMathPolygon` / `surveyToMathPoint` で数学座標系に変換してから処理
- 測量→数学の変換: [simaX, simaY] → [simaY, simaX]（DXF 出力時 XY 反転と同じルール）

### SIMA 立面図の修正

1. **座標軸入れ替え**: `simaToMathPolygon` の [x,y]→[y,x] により境界方向と奥行きの軸が入れ替わる。`transformPointBoundary` に `swapXZ=true` を渡し、出力を `{ x: yp, y: z, z: xp }` とする。
2. **クロップ範囲**: 上方向100mはポリゴン**外側**向き。centroid が +v 側（内側）にあるとき `dot > 0` で AB を反転し、+v を外側に向ける。
